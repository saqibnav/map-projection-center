(async function () {
  ...
  // --- Touch: 1 finger = pan longitude, 2 fingers = tilt equator ---
  let touchMode = null;
  let touchStartX, touchStartY;
  let touchStartLon, touchStartEquatorLat;
  ...
  canvas.addEventListener("touchstart", (e) => {
    ...
    if (e.touches.length === 1) {
      ...
      touchStartLon = centerLon;
    } else if (e.touches.length === 2) {
      touchMode = "tilt";
      touchStartX = (e.touches[0].clientX + e.touches[1].clientX) / 2;
      touchStartY = (e.touches[0].clientY + e.touches[1].clientY) / 2;
      touchStartEquatorLat = equatorLat;
  ...
  canvas.addEventListener("touchmove", (e) => {
    ...
    if (touchMode === "pan" && e.touches.length === 1) {
      ...
      const degPerPixelX = 360 / (width * zoomLevel);
      centerLon = touchStartLon - dx * degPerPixelX;
      updateLabels();
      queueRender();
    } else if (touchMode === "tilt" && e.touches.length === 2) {
      const midY = (e.touches[0].clientY + e.touches[1].clientY) / 2;
      const dy = midY - touchStartY;
      const degPerPixelY = 170 / (height * zoomLevel);
      equatorLat = Math.max(-85, Math.min(85, touchStartEquatorLat + dy * degPerPixelY));
      centerLat = equatorLat;
      ...
      if (lastPinchDist) {
        ...
        zoomLevel = Math.max(1, Math.min(50, zoomLevel * ratio));
      }
      lastPinchDist = dist;
    ...
    // If finger count changes mid-gesture, switch mode
